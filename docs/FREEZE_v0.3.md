...
## تعديل V0.3.1 — بعد مراجعة ملفات AZ Research

**التاريخ**: 2026-09-13
**السبب**: استخلاص ثلاثة دروس من مشروع AZ Operator Geometry
**المرجع**: AZ_Operator_Geometry_v4_Research_Dossier.pdf

### تعديل 1: تمييز Detection vs Estimation

G0.7 يقيس **فقط detection** (هل الطبقتان منفصلتان؟).
**لا يستنتج**: "المعمارية X أفضل من Y" — هذا estimation يحتاج اختباراً آخر.

الاستنتاج المُلزم:
"Policy و Architecture مستقلتان في التصميم" — وليس "معمارية C أفضل".

### تعديل 2: أحجام العينات المعيارية

RUNS_PER_TEST: من 10 → **50**
OPS_PER_RUN: من 50 → **50** (يبقى)
TOTAL_OPS: من 500 → **2500**
الخطأ المعياري لكل قياس: **يُسجَّل صراحة** في التقرير.

لتقليل الخطأ المعياري إلى 0.05 عند p=0.05:
SE = sqrt(0.05 * 0.95 / R) < 0.05
=> R > 19
نستخدم R=50 (هامش 2.5x).

### تعديل 3: آلية تسجيل الفشل

كل Gate يُسجَّل بحالة من ثلاث:
- **CLOSED**: نجح، مرفق الدليل.
- **FAILED**: فشل، مع تحليل السبب.
- **OPEN**: لم يُنفَّذ، مع شرط الإغلاق.

لا حذف. لا "تفسيرات" بعد الفشل.

### ما لن نستورده

- Fisher Information Matrix (خارج نطاق G0.7)
- Bootstrap LR (يحتاج عينات كبيرة لا نملكها)
- Coverage Tests (تحل مشكلة مختلفة)
- أي محتوى من AZ-T, GNSS-ETA, Cosmic Coupling

هذه مشاريع أخرى. لا تُدمج.
